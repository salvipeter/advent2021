...
path(Pos, Caves, Visited, X) :-
    ...
    path(Pos1, Caves, Visited1, X).

adv12(X) :- caves(C), findall(P, path(start, C, [], P), L), length(L, X).
...
path1(end, _, _, _, 1).
path1(Pos, Caves, VisitedOnce, VisitedTwice, X) :-
    Pos \= end,
    ( member(Pos-Pos1, Caves) ; member(Pos1-Pos, Caves) ),
    Pos1 \= start,
    update_visits(Pos1, VisitedOnce, VisitedTwice, VisitedOnce1, VisitedTwice1),
    path1(Pos1, Caves, VisitedOnce1, VisitedTwice1, X).

adv12b(X) :- caves(C), findall(P, path1(start, C, [], none, P), L), length(L, X).
